...
用于展示执行时间线、工具证据、代码 Diff 和记忆状态；它不复制 Agent 循环，也不把现成
Agent 封装成界面。两种前端都通过 `runtime.build_agent()` 装配相同模型、工具和记忆实现。
...
并为未执行的 tool call 补齐取消结果，保持消息协议完整。标准库 HTTP 和当前命令执行器无法
即时中断，所以正在执行的单个操作仍需先返回；界面会明确显示“在当前操作后停止”。
