...
特色：项目不依赖任何 Agent 框架或服务端文件工具，直接实现 OpenAI 兼容的原生 tool calling 循环。Agent 可浏览、读取、搜索、创建和精确编辑本地文件，并执行测试或构建命令。工具错误会结构化回传给模型，使其能够自行修正。文件路径被限制在工作区内，命令具有超时和危险操作拦截；同时具备工具输出截断、上下文裁剪、最大轮数和重复调用保护。
